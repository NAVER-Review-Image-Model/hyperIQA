...
class CustomDataSet(data.Dataset):
    def __init__(self, root, index, transform, patch_num):
        ...
        for i in range(5):
            self.imgname.extend(
                [root + str(i) + '/' + path_image for path_image in os.listdir(root + '/' + str(i))])
            self.mos.extend(
                [i for j in range(len(os.listdir(root + '/' + str(i))))])

        sample = []
        for i, item in enumerate(range(len(self.imgname))):
            for aug in range(patch_num):
                sample.append(
                    (self.imgname[item], self.mos[item]))
        self.samples = sample
        self.transform = transform
